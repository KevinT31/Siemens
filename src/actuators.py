...
# Simular RPi.GPIO si no está disponible (por ejemplo, en Windows)
if sys.platform == "win32" or "linux" not in sys.platform:
...
class ActuatorBase:
    ...
    """
    GPIO_initialized = False  # Clase variable para verificar si GPIO.setmode() ya se ha llamado
    ...
    def __init__(self, gpio_pin, nombre_actuador="Actuador"):
        ...
        if not self.simulation_mode:
            if not ActuatorBase.GPIO_initialized:
                GPIO.setmode(GPIO.BCM)  # Usar numeración BCM
                ActuatorBase.GPIO_initialized = True
...
class ValveControl:
    ...
    def __init__(self, valvulas_config):
        ...
        for nombre, pin in valvulas_config.items():
            nombre_actuador = f"Válvula {nombre.replace('_', ' ').capitalize()}"
            self.valvulas[nombre] = ActuatorBase(pin, nombre_actuador=nombre_actuador)
